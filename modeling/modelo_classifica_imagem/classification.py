...
Reaproveita os GeoTIFFs e o `metadata.json` gerados por
`extract/imagens_satelite/extraction.py` (Step 1) e os índices espectrais de `indices.py`.
Chamado por `step2_classificacao_imagens.py`, um data center por vez.
"""
import json
import os
...
# Valores por padrão só usados se as funções forem chamadas sem `out_dir` explícito —
# step2_classificacao_imagens.py sempre passa `config.RAW_DIR`/`config.LABELS_DIR`.
# RAW_DIR guarda os GeoTIFFs Sentinel-2 + metadata.json (gerados no Step 1); LABELS_DIR guarda
# os rótulos de referência (WorldCover + máscara de vias).
RAW_DIR = 'data/raw'
