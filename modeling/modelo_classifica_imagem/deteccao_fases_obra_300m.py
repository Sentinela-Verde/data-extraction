"""Fluxo PARALELO de teste com imagem menor (300m x 300m) -- mesma logica de
deteccao_fases_obra.py, so lendo de config_obra_300m (saida de
step_classificacao_obra_300m.py). Os limiares abaixo comecam nos MESMOS valores ja
calibrados pro fluxo de 500m, de proposito -- serve de ponto de partida pra comparar,
mas pode precisar recalibrar (caixa menor = menos pixel de contexto por imagem).

A partir da tabela de % por classe/ano gerada por
`step_classificacao_obra_300m.py`, decide em que fase de obra cada (data center, ano) estava —
...
import config_obra_300m as config
