...
Funções reaproveitadas por `step1_extracao_imagens_satelite.py` (que chama
`extract_datacenter_timeseries` uma vez por data center de `data/silver/datacenter_filtrado.csv`)
e reutilizáveis para extrair qualquer outro ponto (sítio de controle, data center avulso) —
basta chamar `extract_datacenter_timeseries` de novo com outro `name_datacenter`/`lat`/`lon`.
"""
import json
import os
...
# Valor por padrão só usado se a função for chamada sem `out_dir` explícito — o step1 sempre
# passa `config.RAW_DIR` (data/raw/imagens_satelite/).
RAW_DIR = 'data/raw'
